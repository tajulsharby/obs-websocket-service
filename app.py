...
# HTTP handler for controlling OBS via HTTP requests
    ...
    try:
        ...
        if command == "START_RECORDING":
            ...
            try:
                ...
                return web.json_response({"error": f"Stopping recording failed: {str(e)}"})
            ...
            try:
                ...
                return web.json_response({"error": f"Snapshot failed: {str(e)}"})
            ...
            try:
                ...
                return web.json_response({"error": f"Saving replay buffer failed: {str(e)}"})

        else:
            return web.json_response({"error": "Unknown command"})
    except json.JSONDecodeError:
        return web.json_response({"error": "Invalid message format"})
    finally:
        obs_service.disconnect()

# Start the WebSocket and HTTP server using aiohttp
app = web.Application()
app.router.add_post('/control', handle_http)  # HTTP endpoint for controlling OBS
